...
class _ChallengeDetailScreenState extends State<ChallengeDetailScreen> {
  ...
  Future<void> _load() async {
    try {
      final challenge = await context.read<ChallengesService>().getBySlug(widget.slug);
      if (!mounted) return;
  ...
  Future<void> _accept() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Challenge accepted. Ready when you are.')),
      );
    } on ApiException catch (e) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    final participation = context.watch<ParticipationProvider>().forChallenge(challenge.id);
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(challenge.shortDescription, style: Theme.of(context).textTheme.bodyMedium),
                  const SizedBox(height: 24),
                  _actionButton(participation),
                  const SizedBox(height: 32),
                  Text('What you will do', style: Theme.of(context).textTheme.titleLarge),
                  const SizedBox(height: 8),
                  Text(challenge.fullDescription, style: Theme.of(context).textTheme.bodyMedium),
                  if (challenge.resources.isNotEmpty) ...[
                    const SizedBox(height: 32),
                    Text('Resources', style: Theme.of(context).textTheme.titleLarge),
                    const SizedBox(height: 12),
                    ...challenge.resources.map((r) => Card(
      ...
      return SizedBox(
        ...
        child: ElevatedButton(
          onPressed: () => Navigator.of(context).push(
            MaterialPageRoute(builder: (_) => SubmitEvidenceScreen(participation: participation)),
          ),
